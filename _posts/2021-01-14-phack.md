...
How to p-hack a robust result {#how-to-p-hack-a-robust-result .unnumbered .unnumbered}
=============================

Economists want to show that our results our robust. Consider Table
[\[tab1\]](#tab1){reference-type="ref" reference="tab1"} below: Column 1
contains the baseline model, with no covariates, and Column 2 controls
for $z$. Because the coefficient on $X$ is stable and significant across
columns, we say that our result is robust.
...
\centering
  ...
  \[-1.8ex\] Observations         1,000             1,000
  Adjusted R$^{2}$                0.005             0.048
                                              
  \[-1.8ex\] *Note:*                          
  ------------------------- ----------------- -----------------

  : Robust results[]{label="tab1"}

The twist: I p-hacked this result, using data where the true effect of
$X$ is zero.

In this post, I will show that, under certain conditions, it is easy to
p-hack a robust result like this. The key is to use control variables
that have a low partial-$R^{2}$. These variables have little effect on
our main coefficient when excluded from the regression, and hence little
effect when included. In contrast, controls with high partial-$R^{2}$
are more likely to kill a false positive, because they have more
...
generated according to $$\label{eq1}
  % \begin{split}
  y_{i} = \sum_{k=1}^{K} \beta_{k} X_{k,i} + \gamma z_{i} + \varepsilon_{i}.% \\
  % & =\gamma z_{i} + \varepsilon_{i}.
  % \end{split}$$ We have $K$ potential treatment variables, $X_{1,i}$
...
$$y_{i} = \alpha_{1,k} + \beta_{1,k}X_{k,i} + \nu_{i}.
...
$$y_{i} = \alpha_{2,k^{*}} + \beta_{2,k^{*}}X_{k^{*},i} + \gamma z_{i} + \varepsilon_{i}$$
...
\nu_{i} &= \sum_{j \neq k}^{K} \beta_{1,j} X_{j,i} + \gamma z_{i} + \varepsilon_{i} \\
  &= \gamma z_{i} + \varepsilon_{i}
\end{split}$$ We estimate the slope coefficient as
$$\hat{\beta}_{1,k} = \frac{\widehat{Cov}(X_{k},y)}{\widehat{Var}(X_{k})} = \frac{\gamma \widehat{Cov}(X_{k},z) + \widehat{Cov}(X_{k},\varepsilon)}{\widehat{Var}(X_{k})}.$$
...
1.  $\gamma \widehat{Cov}(X_{k},z)$

2.  $\widehat{Cov}(X_{k},\varepsilon)$
...
$$\hat{\beta}_{2,k} = \frac{\widehat{Cov}(\hat{u},y)}{\widehat{Var}(\hat{u})} = \frac{\widehat{Cov}((X_{k} - \hat{\lambda}_{1} z),\varepsilon)}{\widehat{Var}(\hat{u})} = \frac{\widehat{Cov}(X_{k},\varepsilon) - \hat{\lambda}_{1} \widehat{Cov}(z,\varepsilon)}{\widehat{Var}(\hat{u})}.$$
...
![Shares of $\hat{\beta}_{1,k}$, varying with
$\gamma$[]{label="fig0"}](/home/michael/Dropbox/blog/p-hack/output/b0_shares_signif.pdf){#fig0}

Figure [\[fig0\]](#fig0){reference-type="ref" reference="fig0"} repeats
this process for a range of $\gamma$'s. I plot the shares of
...
$$y_{i} = \sum_{k=1}^{K} \beta_{k,i} X_{k,i} + \gamma z_{i} + \varepsilon_{i},$$
where $\beta_{k,i} \sim N(0,1)$.

Then, when regressing $y$ on $X_{k}$, we have
$$y_{i} = \alpha_{1,k} + \delta_{1,k} X_{k,i} + v_{i},$$ where
$$v_{i} = -\delta_{1,k} X_{k,i} + \beta_{k,i} X_{k,i} + \sum_{j \neq k}^{K} \beta_{j,i} X_{j,i} + \gamma z + \varepsilon_{i}.$$
...
The estimated slope coefficient is $$\begin{split}
...
$$\hat{\delta}_{1,k} \rightarrow \frac{E[w_{i} \beta_{k,i}]}{E[w_{i}]},$$
...
1.  $\sum_{j=1}^{K} \widehat{Cov}(X_{k,i},\beta_{j,i}X_{j,i})$

2.  $\gamma \widehat{Cov}(X_{k},z)$

3.  $\widehat{Cov}(X_{k},\varepsilon)$
...
regression, we have $$\begin{split}
  ...
  &= \frac{\sum_{j=1}^{K} \widehat{Cov}(X_{k,i},\beta_{j,i}X_{j,i}) + \widehat{Cov}(X_{k,i},\varepsilon_{i})}{\widehat{Var}(\hat{u_{i}})} - \hat{\lambda}_{1} \frac{\left[ \sum_{j=1}^{K} \widehat{Cov}(z_{i},\beta_{j,i}X_{j,i}) + \widehat{Cov}(z_{i},\varepsilon_{i}) \right] }{\widehat{Var}(\hat{u_{i}})}
  % &= \frac{\sum_{j=1}^{K} \widehat{Cov}(X_{k,i},\beta_{j,i}X_{j,i}) + \widehat{Cov}(X_{k_{i}},\varepsilon_{i})}{\widehat{Var}(\hat{u}_{i})} - \hat{\lambda}_{1} \frac{\left[ \sum_{j=1}^{K} \widehat{Cov}(z_{i},\beta_{j,i}X_{j,i}) + \widehat{Cov}(z_{i},\varepsilon) \right] }{\widehat{Var}(\hat{u}_{i})}
\end{split}$$ Here $\hat{u}$ is the residual from a regression of
...
![Shares of $\hat{\delta}_{1,k}$ and
robustness[]{label="fig1"}](/home/michael/Dropbox/blog/p-hack/output/bhet_shares.pdf){#fig1}

As shown in Figure [\[fig1\]](#fig1){reference-type="ref"
reference="fig1"}, when $\gamma$ is small, most of the weight in
